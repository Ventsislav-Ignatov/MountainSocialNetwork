namespace MountainSocialNetwork.Services.Data.Tests
{
    ...
    {
        [Fact]
        public void CreateCommentShouldWorkCorrectly()
        {
            ...
            service.Create(comment.ArticleId, comment.UserId, comment.Content, comment.ParentId);
        ...
        [Fact]
        public void IsInSamePostReturnTrue()
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
              .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString()).Options;
            var dbContext = new ApplicationDbContext(options);
